(function () {
  ...
  function computeFeeNaira() {
    ...
    if (promo.discountType === "percent") {
      return Math.max(0, Math.round(base - base * ((promo.discountValue || 0) / 100)));
    }
    return base;
  }
  ...
  if (feeNaira <= 0) {
    ...
    payBtn.addEventListener("click", function () {
      ...
      var payerEmail = emailInput ? emailInput.value.trim() : "";
      ...
      var payerName = ((firstNameInput ? firstNameInput.value.trim() : "") + " " + (lastNameInput ? lastNameInput.value.trim() : "")).trim();
  ...
  // Trigger map init — container is visible at this point
  if (typeof window.ceaShowMap === "function") {
    window.ceaShowMap();
  }
})();
